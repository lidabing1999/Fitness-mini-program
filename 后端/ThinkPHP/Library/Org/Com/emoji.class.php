...
$GLOBALS['emoji_maps'] = array(
    ...
    'unified_to_softbank' => array(
        ...
        "$text"=>"[\xe3\x83\x81\xe3\x82\xa7\xe3\x83\x83\xe3\x82\xaf\xe3\x83\x9e\xe3\x83\xbc\xe3\x82\xaf]", "\xe2\x9c\x8a"=>"\xee\x80\x90", "\xe2\x9c\x8b"=>"\xee\x80\x92", "\xe2\x9c\x8c"=>"\xee\x80\x91", "\xf0\x9f\x91\x8a"=>"\xee\x80\x8d",
...
class emoji {
        ...
        #
    function emoji_docomo_to_unified($text){ return $this->emoji_convert($text, 'docomo_to_unified'); }
    function emoji_kddi_to_unified($text){ return $this->emoji_convert($text, 'kddi_to_unified'); }
    function emoji_softbank_to_unified($text){ return $this->emoji_convert($text, 'softbank_to_unified'); }
    function emoji_google_to_unified($text){ return $this->emoji_convert($text, 'google_to_unified'); }
    #
    # functions to convert unified data into an outgoing format
    #

    function emoji_unified_to_docomo($text){ return $this->emoji_convert($text, 'unified_to_docomo'); }
    function emoji_unified_to_kddi($text){ return $this->emoji_convert($text, 'unified_to_kddi'); }
    function emoji_unified_to_softbank($text){ return $this->emoji_convert($text, 'unified_to_softbank'); }
    function emoji_unified_to_google($text){ return $this->emoji_convert($text, 'unified_to_google'); }
    function emoji_unified_to_html($text){ return $this->emoji_convert($text, 'unified_to_html'); }
    function emoji_html_to_unified($text){ return $this->emoji_convert($text, 'html_to_unified'); }
    ...
    private function emoji_get_name($unified_cp){
